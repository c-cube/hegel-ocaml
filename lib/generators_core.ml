...
(** [values_core ~pool_id ~find ~remove ~is_empty ~consume] builds an
    ...
    Carries no printer (the output type is the caller's). Table-agnostic: see
    {!Make_pool} for the public, [Hashtbl.S]-backed entry point. *)
...
(** A ready-made int-keyed hashtable module, for callers who just need a
    concrete table to pass to {!Make_pool}. *)
...
(** [Make_pool (Tbl)] specializes {!values_core}/{!resolve_draw_core} to a
    concrete int-keyed hashtable module, so the public API never bakes in a
    specific [Hashtbl] implementation: callers pick their own via
    [Stdlib.Hashtbl.Make] or {!Int_table}. *)
module Make_pool (Tbl : Stdlib.Hashtbl.S with type key = int) = struct
  let resolve_draw values ~consume variable_id =
    resolve_draw_core
      ~find:(Tbl.find_opt values)
      ~remove:(Tbl.remove values)
      ~consume
      variable_id
  ;;
...
let draw_named
  ...
  | Printable { core = Function { build }; sexp_of } ->
    ...
    then Internal.note tc (Printf.sprintf "%s = %s" name (Sexp.to_string_hum (sexp_of value)));
    value
