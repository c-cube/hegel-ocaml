...
(** [values_core ~pool_id ~find ~remove ~is_empty ~consume] is the
    table-agnostic primitive behind {!Make_pool}: it builds a generator that
    picks a value from the engine pool [pool_id], resolving the drawn id via
    [find] and, when [consume], deleting it via [remove]; [is_empty] reports
    whether the backing store is currently empty. Exposed so a table type that
    doesn't conform to [Stdlib.Hashtbl.S] (e.g. [Core.Hashtbl.t], used by
    [hegel.core]) can still build a pool generator, by supplying its own
    [find]/[remove]/[is_empty] closures instead of instantiating {!Make_pool}. *)
val values_core
  :  pool_id:int
  -> find:(int -> 'a option)
  -> remove:(int -> unit)
...
(** [resolve_draw_core ~find ~remove ~consume id] is the table-agnostic
    primitive behind {!Make_pool}'s [resolve_draw]. See {!values_core}. *)
...
(** A ready-made int-keyed hashtable module satisfying
    [Stdlib.Hashtbl.S with type key = int], for callers who just need a
    concrete table to pass to {!Make_pool} without writing their own
    [Stdlib.Hashtbl.Make(Int)] instantiation. *)
module Int_table : Stdlib.Hashtbl.S with type key = int

(** [Make_pool (Tbl)] specializes the pool-drawing machinery to a concrete
    int-keyed hashtable module [Tbl] (typically {!Int_table} or a caller's own
    [Stdlib.Hashtbl.Make(Int)]), so no specific [Hashtbl] implementation is
    baked into this library's API. *)
module Make_pool (Tbl : Stdlib.Hashtbl.S with type key = int) : sig
  ...
  (** [pool_values ~pool_id ~values ~consume] builds a generator that picks a
      ...
      the pool. Carries no printer, so it is {!unprintable}. *)
  val pool_values
    :  pool_id:int
    -> values:'a Tbl.t
    -> consume:bool
...
(** [with_printer sexp_of gen] attaches (or replaces) [gen]'s printer, yielding
    ...
    The printer is any ['a -> Sexplib0.Sexp.t]. With [ppx_sexp_conv] in your
    preprocessor, [\[%sexp_of: t\]] is the idiomatic shorthand for that function
...
(** Internal plumbing consumed by the [ppx_hegel_generator] deriver and
    white-box tests. Not for direct use — no stability guarantees. *)
